# python attack_lane_id.py --subset-ratio 0.05 --epochs 5
...
NUM_LANE_CLASSES = 7   # private attribute: lane ID (lanes 1–7, 0-indexed; US-101 has auxiliary ramp lanes)
LANE_LABELS      = ['Lane 1', 'Lane 2', 'Lane 3', 'Lane 4', 'Lane 5', 'Lane 6', 'Lane 7']
...
REPR_DIM  = _soc_emb + MODEL_ARGS['dyn_embedding_size']   # 80 + 32 = 112
...
class ngsimDatasetWithLaneId(ngsimDataset):
    """
    Extends ngsimDataset to return the vehicle's lane ID (0-indexed, 0–5) as an
    additional output — the private lane attribute.

    D[idx, 5] holds the 1-indexed lane ID (1–6); we subtract 1 to get 0–5.
    """

    def __getitem__(self, idx):
        lane_label = int(self.D[idx, 5]) - 1   # 0-indexed lane (0–5)
        hist, fut, neighbors, lat_enc, lon_enc = super().__getitem__(idx)
        return hist, fut, neighbors, lat_enc, lon_enc, lane_label
    ...
    def collate_fn(self, samples):
        ...
        lane_batch    = torch.zeros(len(samples), dtype=torch.long)

        count = 0
        for sampleId, (hist, fut, nbrs, lat_enc, lon_enc, lane_id) in enumerate(samples):
...
class LaneAttackMLP(nn.Module):
    """
    6-layer MLP attack model: z → lane ID (6 classes, lanes 1–6).
    ...
    def __init__(self, repr_dim: int = REPR_DIM, num_classes: int = NUM_LANE_CLASSES):
        super().__init__()
...
def extract_representations(encoder, dataset: ngsimDatasetWithLaneId, indices: list,
                             device, batch_size: int = EXTRACT_BATCH_SIZE,
                             split_name: str = '') -> TensorDataset:
    """Extract (z, lane_id) pairs for the given list of dataset indices."""
    ...
    label_counts = np.zeros(NUM_LANE_CLASSES, dtype=int)
    for idx in indices:
        lane_id = int(dataset.D[idx, 5]) - 1
        label_counts[lane_id] += 1

    tag = f' [{split_name}]' if split_name else ''
    print(f'  {len(indices)} samples{tag}')
    for i, cnt in enumerate(label_counts):
        print(f'    {LANE_LABELS[i]}: {cnt} ({cnt/len(indices)*100:.1f}%)')

    z_list, label_list = [], []
    encoder.eval()
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            hist, nbrs, mask, _, _, _, _, lane = batch
            z = encoder.get_representation(hist.to(device), nbrs.to(device), mask.to(device))
            z_list.append(z.cpu())
            label_list.append(lane.cpu())
            if (batch_idx + 1) % 50 == 0:
...
def train_attack_model(
    ...
    model     = LaneAttackMLP(REPR_DIM, NUM_LANE_CLASSES).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode='min', factor=0.5, patience=5, min_lr=1e-6,
    )
    # Class-balanced loss to handle lane imbalance
    label_counts  = torch.bincount(train_ds.tensors[1], minlength=NUM_LANE_CLASSES).float()
    class_weights = (label_counts.sum() / (NUM_LANE_CLASSES * label_counts)).to(device)
...
def evaluate_attack_model(
    ...
    per_class_acc = {}
    for c in range(NUM_LANE_CLASSES):
        mask = all_labels == c
...
def plot_training_and_accuracy(train_history: dict, results: dict, run_dir: str) -> None:
    ...
    ax.axhline(100 / NUM_LANE_CLASSES, color='red', linestyle=':', linewidth=1.5,
               label=f'Random baseline ({100/NUM_LANE_CLASSES:.1f}%)')
    ...
    plt.suptitle('Attack Model Training Curves — Lane ID Inference', fontsize=13, fontweight='bold')
    plt.tight_layout()
...
def plot_confusion_matrix(results: dict, run_dir: str) -> None:
    cm      = confusion_matrix(results['all_labels'], results['all_preds'],
                               labels=list(range(NUM_LANE_CLASSES)))
    ...
    for ax, data, title, fmt in zip(
        ...
        sns.heatmap(data, annot=True, fmt=fmt, cmap='Blues',
                    xticklabels=LANE_LABELS, yticklabels=LANE_LABELS,
                    ax=ax, linewidths=0.5)
        ax.set_xlabel('Predicted lane'); ax.set_ylabel('True lane'); ax.set_title(title)

    plt.suptitle('Attack Model Confusion — Lane ID Inference',
    ...
    print(classification_report(results['all_labels'], results['all_preds'],
                                target_names=LANE_LABELS, digits=4))


def print_summary(results: dict, fl_checkpoint: str) -> None:
    random_baseline = 1.0 / NUM_LANE_CLASSES   # ~16.7% for 6-class
    attack_acc      = results['accuracy']
    leakage_gap     = attack_acc - random_baseline

    if leakage_gap > 0.30:
        severity = 'HIGH';     comment = 'Strong lane signal — privacy protection recommended.'
    elif leakage_gap > 0.10:
        severity = 'MODERATE'; comment = 'Meaningful leakage present — privacy protection advisable.'
    else:
        severity = 'LOW';      comment = 'Leakage close to random — limited lane information.'
    ...
    print(f'  Private attribute: lane ID (7 classes: lanes 1–7)')
    print(f'  Attack model     : LaneAttackMLP (6-layer MLP, BN+ReLU+Dropout)')
    print()
    print(f'  Random baseline  : {random_baseline*100:.2f}%  (1/7)')
    ...
    for c in range(NUM_LANE_CLASSES):
        ...
        print(f'    {flag} {LANE_LABELS[c]}: {acc*100:.2f}%  (gap: +{gap*100:.2f}%)')
    print('=' * 60)
    print()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Privacy attack: lane ID inference from FL representations')
...
def main() -> None:
    ...
    dataset   = ngsimDatasetWithLaneId(args.train_data)
    ...
    run_dir = make_run_dir(LaneAttackMLP)
    ...
    print(f'  Random Baseline         : {100/NUM_LANE_CLASSES:.2f}%')
    print(f'  Leakage Gap             : +{(results["accuracy"] - 1/NUM_LANE_CLASSES)*100:.2f}%')
    print(f'{"─"*50}')
    print('\nPer-lane accuracy:')
    for c, acc in results['per_class_acc'].items():
        bar = '█' * int(acc * 30)
        print(f'  {LANE_LABELS[c]:<8} {acc*100:6.2f}%  {bar}')
